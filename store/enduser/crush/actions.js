...
export default {
  async getUserCrushs({ commit }, username) {
    commit("partials/setLoading", true, { root: true });
    const dataObj = [];
    const snap = await getCollectionwithid("crushs")
      .where("crushsOf", "==", String(username))
      .get()
      .then(res => {
        let crush = res.docs.map(doc => {
          Object.assign(dataObj, doc.data(), { id: doc.id });
        });
        commit("setUserCrushs", dataObj);
      });
